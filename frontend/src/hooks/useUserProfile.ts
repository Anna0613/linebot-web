...
export const useUserProfile = () => {
  ...
  const updateDisplayName = useCallback(async (newDisplayName: string) => {
    try {
      const response = await apiClient.updateUserProfile({ display_name: newDisplayName });
      
      if (response.success) {
        setUser(prev => prev ? { ...prev, display_name: newDisplayName } : null);
        toast({
          title: "更新成功",
          description: "顯示名稱已更新",
        });
      ...
      toast({
        variant: "destructive",
        title: "更新失敗",
        description: error.message || "無法更新顯示名稱",
      });
      return false;
    }
  }, [apiClient, toast]);
  ...
  const uploadAvatar = useCallback(async (file: File) => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "頭像上傳失敗",
        description: error.message || "無法上傳頭像",
      });
      return false;
    } finally {
  ...
  const deleteAvatar = useCallback(async () => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "刪除失敗",
        description: error.message || "無法刪除頭像",
      });
      return false;
    } finally {
      setAvatarLoading(false);
  ...
  const deleteAccount = useCallback(async () => {
    try {
      ...
      toast({
        variant: "destructive",
        title: "刪除失敗",
        description: error.message || "無法刪除帳號",
      });
      return false;
    }
  }, [apiClient, toast]);
